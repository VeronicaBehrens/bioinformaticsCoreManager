#!/bin/bash
#SBATCH --job-name=intPros
#SBATCH --output=logs_04_intronProspector/chr22_log.out
#SBATCH --error=logs_04_intronProspector/chr22_log.err
#SBATCH --time=0:15:00
...
module load gcc/13.3.0    # need for intronProspector to run

SAMPLE=CHAf-D1_humanONLY
SAMPLE=CHAf-D1_chimpONLY

INPUT_BAM=${SAMPLE}.fl.aligned_hg38.sorted.MAPq0removed_chr22ONLY.bam
OUTPUT_INTRON_JUNCTION_BED=${SAMPLE}.fl.aligned_hg38.sorted.MAPq0removed_chr22ONLY_intronJunctions.bed
GENOME_FASTA=/labs/kingsley/jsong4/ref/hg38/hg38.fa

##### DISCOVER SPLICE JUNCTIONS SUPPORTED BY LONG-READ DATA #####
intronProspector \
--genome-fasta=$GENOME_FASTA \
--intron-bed6=$OUTPUT_INTRON_JUNCTION_BED \
...
#--min-confidence-score=0.0  # necessary for long-reads
#--min-anchor-length=0       # don't require any exact matches around the splice site (default is 8)
#--allow-anchor-indels       # allow indels in the exons
#--sj-filter=all             # keep all junctions, not just canonical splice junctions
